...
EXTRACTORS = {
    ...
    "ids": AsyncVotacoesIdsExtractor,
    "orientacoes": AsyncVotacoesOrientacoes,
    "votos": AsyncVotosExtractor,
}

DEPENDENCIES = {
    "ids":         {"votacoes": "votacoes"},
    "orientacoes": {"votacoes": "votacoes"},
    "votos":       {"votacoes": "votacoes"},
}
...
async def _run(event: dict):
    ...
    bundle_name = os.getenv("BUNDLE", "votacoes")
...
def _write_output(
    ...
    if dest_type == "s3":
        ...
        output_path = Path(destination.get("path", f"/tmp/votacoes/{extractor_name}.json"))
